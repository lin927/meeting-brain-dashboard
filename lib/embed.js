...
export async function indexChunkIds(ids, { dbPath, model, dtype } = {}) {
  ...
  const vecs = await embed(rows.map((c) => c.chunk_text), { model, dtype });
  for (let i = 0; i < rows.length; i++) {
    updateChunkVector(db, rows[i].id, JSON.stringify(vecs[i]));
  }
  db.close();
  return rows.length;
}
