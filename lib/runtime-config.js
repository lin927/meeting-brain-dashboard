// 本机设置：大模型（问/总结）、公司 RAGFlow（发布）、工时项目参考（称呼同步）。
...
/** 工时 HTWorkbuddy 项目参考接口。地址和令牌只存在本机，仓库不写默认值。 */
export function resolveHtConfig(db) {
  const owned = db || open();
  const envUrl = process.env.HTWORKBUDDY_BASE_URL || '';
  const envKey = process.env.HTWORKBUDDY_PAT || '';
  const url = stripSlash(envUrl || getMeta(owned, 'ht_url') || '');
  const apiKey = envKey || getMeta(owned, 'ht_key') || '';
  return {
    url,
    apiKey,
    keySet: !!apiKey,
...
export function saveHtConfig(db, body) {
  if (body.url !== undefined) setMeta(db, 'ht_url', stripSlash(body.url));
  if (body.apiKey !== undefined && String(body.apiKey).trim()) {
    setMeta(db, 'ht_key', String(body.apiKey).trim());
  }
}

export function htPublicView(cfg) {
  return {
    url: cfg.url,
    keySet: cfg.keySet,
    envLocked: cfg.envLocked,
...
/** 改标题、「记录」是否写回来源侧听记。未配置时默认打开。腾讯会议和导入场次始终只改本机。 */
export function resolveWriteback(db) {
  const owned = db || open();
  const raw = getMeta(owned, 'minutes_writeback');
  return { enabled: raw !== '0' };
}

export function saveWriteback(db, body) {
  if (!body || body.enabled === undefined) return;
  setMeta(db, 'minutes_writeback', body.enabled ? '1' : '0');
}
