...
function isGone(err) {
  ...
  return /does not have the document|doesn't exist|not found|不存在|不拥有/i.test(s);
}
...
export async function updateDocument(kb, datasetId, documentId, body) {
  return ragflowRequest(kb, {
    method: 'PATCH',
    path: `/api/v1/datasets/${encodeURIComponent(datasetId)}/documents/${encodeURIComponent(documentId)}`,
    json: body,
  });
}
...
export async function findMeetingDocuments(kb, datasetIds, meetingId, known = {}, opts = {}) {
  ...
  if (known.docId && known.datasetId) {
    try {
      const docs = await listDocuments(kb, known.datasetId, { id: known.docId, page: 1, page_size: 5 });
      const hit = docs.find((d) => d.id === known.docId) || (docs[0] && docs[0].id === known.docId ? docs[0] : null);
