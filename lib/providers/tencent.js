...
import { parseJsonText, unwrapCli, asArray, pick, parseTime, lookbackRange, dayWindows } from '../cli-json.js';
...
function flattenRecords(rows) {
  ...
  for (const row of rows) {
    ...
    const chosen = files.length ? [preferFile(files)] : [];
...
function parseParagraphs(json) {
  ...
  return list.map((p) => {
    ...
    return { speaker: speakerName(p), text: paragraphText(p) };
  }).filter((p) => p.text);
}
...
async function ingestRecord(db, meta, { strict = false, log = () => {} } = {}) {
  const d = await fetchRecord(meta, { strict });
  return ingestNormalized(db, {
    ...
    remote_json: {
      kind: 'record',
      record_file_id: meta.remote_id,
      meeting_id: meta.meeting_id || '',
      meeting_record_id: meta.meeting_record_id || '',
    },
    title: meta.title,
...
async function ingestYuanbao(db, item, { log = () => {} } = {}) {
  ...
  if (existing) {
    ...
    const start = existing.start_time || parsed.start_time;
    const summary = existing.summary || parsed.summary || '';
    if (start !== existing.start_time || (parsed.summary && !existing.summary)) {
      ingestNormalized(db, {
...
export async function refresh(meeting) {
  ...
  try {
    if (extra.kind === 'yuanbao') {
      ...
      return { ok: true, title: meeting.title, keptRecord: false, keptTranscript: false, transcriptCount: 0 };
    }
