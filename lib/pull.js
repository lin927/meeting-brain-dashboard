...
export function pull({ dbPath, maxUuid = 300, skipTranscript = false, only = null, skipExisting = true, quiet = false } = {}) {
  ...
  // 1. 全量拉取所有可访问的听记（按版本选语法）
  const style = detectCliStyle();
  let listJson;
  if (style === 'new') {
    // v1.0.58+：+list-all（返回 minutes[]，含 taskUuid）
    listJson = runDws(['minutes', '+list-all', '--limit', '100', '--format', 'json'], { soft: true });
  } else {
    // v1.0.5x 旧版：list all（返回 result.itemList，含 uuid）
  ...
  log(`[pull] 发现 ${all.length} 条听记（${style === 'new' ? '+list-all' : 'list all'} 全量）`);
  ...
  for (const meta of targets) {
    ...
    if (!skipTranscript) {
      ...
      // 新版：transcript.paragraphList；旧版：result.paragraphList / result.paragraphs / 数组
