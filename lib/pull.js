...
import { open, getMeeting, listDeletedUuids } from './db.js';
import { ingestNormalized, writeTranscriptChunks } from './ingest.js';
...
function applyOrigins(db, origin, log) {
  ...
  const rows = db.prepare(`SELECT task_uuid, source, provider FROM meetings`).all();
  const upd = db.prepare(`UPDATE meetings SET source = ? WHERE task_uuid = ?`);
  let nMine = 0;
  let nShared = 0;
  let nChanged = 0;
  for (const row of rows) {
    if (row.source === 'import' || String(row.task_uuid || '').startsWith('import-')) continue;
    if (row.provider && row.provider !== 'dingtalk') continue;
...
async function ingestMeeting(db, meta, { style, origin, skipTranscript = false, strict = false, log = () => {} } = {}) {
  ...
  if (!skipTranscript) {
    try {
      plist = await fetchTranscriptParagraphs(style, meta.task_uuid, { strict });
    } catch (e) {
      if (strict) throw e;
  ...
  const r = ingestNormalized(db, {
    task_uuid: meta.task_uuid,
    provider: 'dingtalk',
    remote_id: meta.task_uuid,
    ...
    todos: todosOk ? todos : undefined,
    transcript: skipTranscript ? undefined : plist,
  }, { skipTranscript, log });
  return { ...r, todosUpdated: todosOk };
}
...
export async function refreshMeeting({ taskUuid } = {}) {
  ...
  try {
    ...
    if (m.source === 'import' || id.startsWith('import-') || m.provider === 'import') {
      throw new Error('导入场次没有对应的听记');
    }
    if (m.provider && m.provider !== 'dingtalk') {
      throw new Error('请用对应来源重拉');
    }
    const style = await detectCliStyle();
  ...
  const targets = (only && only.length)
    ...
    : db.prepare(`
        ...
        WHERE IFNULL(m.source, '') != 'import'
          AND IFNULL(m.provider, 'dingtalk') IN ('', 'dingtalk')
