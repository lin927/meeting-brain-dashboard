...
async function runDwsRaw(args, opts = {}) {
  try {
    const { stdout } = await execDws(args);
...
async function fetchTranscriptParagraphs(style, taskUuid) {
  try {
    if (style === 'new') {
      // +transcript 默认追完 nextToken；禁止 --single-page（一页最多 50 段）
      const txRes = await runDws(['minutes', '+transcript', '--id', taskUuid, '--format', 'json']) ?? {};
    ...
    const short = String(e.message || e).replace(/\s+/g, ' ').slice(0, 80);
...
export async function pull({ dbPath, maxUuid = 300, skipTranscript = false, only = null, skipExisting = true, quiet = false, onProgress = null } = {}) {
  ...
  for (const meta of targets) {
    const style = await detectCliStyle();
    // 每个产物单独调用，失败互不影响
    const fetchOne = async (artifact, extra = []) => {
      try {
        if (style === 'new') {
          // v1.0.58+：+detail --artifacts
          return await runDws(['minutes', '+detail', '--id', meta.task_uuid,
        ...
        return await runDws(['minutes', 'get', legacyCmd, '--id', meta.task_uuid, '--format', 'json', ...extra]) ?? {};
      } catch (e) {
    ...
    if (!skipTranscript) {
      if (prev && prev.transcript_edited_at) {
        ...
        const plist = await fetchTranscriptParagraphs(style, meta.task_uuid);
