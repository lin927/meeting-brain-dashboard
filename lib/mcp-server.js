...
function createServer(port) {
  ...
  server.registerTool('meeting_list', {
    ...
    inputSchema: z.object({
      ...
      provider: z.enum(['dingtalk', 'feishu', 'tencent', 'import']).optional().describe('听记来源'),
  ...
  server.registerTool('meeting_add_todo', {
    title: '登记待办',
    description: '在本机一场会议上登记待办。不写到来源侧。',
    inputSchema: z.object({
  ...
  server.registerTool('meeting_update_todo', {
    title: '改待办',
    description: '关闭、打开或改本机待办的标题/责任人/归类/时限。不写到来源侧。',
    inputSchema: z.object({
  ...
  server.registerTool('meeting_summarize', {
    title: '生成本机总结',
    description: '根据逐字稿生成本机决策记录，写入 deep_summary，不改来源侧听记。若已有总结，必须 confirm_overwrite=true，否则会提示覆盖。',
    inputSchema: z.object({
      id: z.string().min(1).describe('会议 taskUuid'),
    ...
    if (String(d.deepSummary || '').trim() && !confirm_overwrite) {
      return asError('本机已有总结，再生成会覆盖（含改过的内容）。确认请带 confirm_overwrite=true。来源侧听记不会改。')
    }
  ...
  server.registerTool('meeting_pull', {
    title: '更新听记',
    description: '从已登录的钉钉、飞书、腾讯拉新听记到本机，等同界面「更新」。正在拉时会返回请稍候。',
    inputSchema: z.object({
