...
import { projectFromHit } from './project.js';
...
function matchGlossaryProject(ctx, glossary) {
  for (const p of (glossary && glossary.projects) || []) {
    const needles = unique([p.name, ...(p.aliases || []), p.code]);
    if (needles.some((n) => includesTerm(ctx.title, n) || includesTerm(ctx.record, n))) {
      return { type: '项目', tags: [p.name], project: p.name, projectCode: p.code || '', via: 'glossary:' + p.name };
    }
  }
  return null;
}
...
export function matchMeeting(m, cfg, glossary) {
  ...
  for (const rule of cfg.rules || []) {
    if (ruleMatches(rule, ctx, g)) {
      const hit = { type: rule.type, tags: rule._tags || splitTerms(rule.tags), via: 'rule:' + rule.id };
      if (rule.type === '项目') {
        const fromTags = projectFromHit(hit, g);
        const fromText = fromTags.name ? null : matchGlossaryProject(ctx, g);
        const name = fromTags.name || (fromText && fromText.project) || '';
        if (name) {
          hit.project = name;
          hit.projectCode = fromTags.code || (fromText && fromText.projectCode) || '';
          hit.tags = unique([name, ...(hit.tags || [])]);
        }
      }
      return hit;
    }
...
export function applyClassify(db, taskUuid, { onlyEmpty = false, force = false } = {}) {
  ...
  if (hit.type === '项目') {
    const resolved = projectFromHit(hit, loadGlossary());
    if (resolved.name) {
      patch.project_name = resolved.name;
      patch.project_code = resolved.code || hit.projectCode || '';
    }
  } else {
    patch.project_name = '';
    patch.project_code = '';
  }
  updateMeetingFields(db, taskUuid, patch);
  return { ok: true, taskUuid, title: m.title, type: hit.type, tags, project: patch.project_name || '', via: hit.via };
}
