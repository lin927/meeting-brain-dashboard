// DeepSeek RAG 问答：本地语义检索命中会议块 -> 把命中内容作为上下文交给 DeepSeek 生成答案。
// 数据不出本机，只有命中文本与问题发往云端模型。

import { semanticSearch } from './embed.js';
import { open, listActions, getMeeting } from './db.js';
import { todosByRange } from './overview.js';
import {
  loadGlossary, loadSummarizePrompt, extractSpeakers, resolveGlossary, formatSummarizeInputs,
} from './glossary.js';
import { resolveLlmConfig } from './runtime-config.js';

// 读取系统代理（HTTPS_PROXY/HTTP_PROXY/ALL_PROXY）。Windows 公司内网常需走代理访问外网，
// Node 全局 fetch（undici）不读这些环境变量，这里手动实现以兼容代理环境。
function getProxyUrl() {
  const candidates = ['HTTPS_PROXY', 'https_proxy', 'HTTP_PROXY', 'http_proxy', 'ALL_PROXY', 'all_proxy'];
  for (const k of candidates) {
    const v = process.env[k];
    if (v && v.trim()) return v.trim();
  }
  return null;
}

// 走代理的 fetch：支持 http 代理（含 https 目标的 CONNECT 隧道）与直连（返回 {status, text}）。
// Windows/公司内网常需 HTTP 代理访问外网；Node 全局 fetch 不读代理环境变量，这里手动实现。
export async function proxiedFetch(url, options) {
  const proxy = getProxyUrl();
  const parsed = new URL(url);
  const isHttps = parsed.protocol === 'https:';
  const httpMod = await import('node:http');
  const httpsMod = await import('node:https');

  const body = options.body ?? '';
  const headers = { ...(options.headers || {}) };
  if (body && !headers['Content-Length'] && !headers['content-length']) {
    headers['Content-Length'] = String(
      typeof body === 'string' ? Buffer.byteLength(body) : Buffer.byteLength(body),
    );
  }

  return new Promise((resolve, reject) => {
    let req;
    if (proxy && proxy.startsWith('http://') && isHttps) {
      // https 目标 + http 代理：先 CONNECT 建立隧道，再在隧道内做 TLS。
      // 直接对代理发明文 http 请求到 https 端口会被 DeepSeek 拒绝（400 plain http to https port）。
      const p = new URL(proxy);
      const connectReq = httpMod.request({
        hostname: p.hostname,
        port: p.port || 80,
        method: 'CONNECT',
        path: `${parsed.hostname}:${parsed.port || 443}`,
        headers: { Host: `${parsed.hostname}:${parsed.port || 443}` },
      });
      connectReq.on('connect', async (res, socket) => {
        if (res.statusCode !== 200) {
          reject(new Error(`代理 CONNECT 失败: ${res.statusCode}`));
          return;
        }
        const tlsMod = await import('node:tls');
        const tlsSocket = httpsMod.request({
          hostname: parsed.hostname,
          port: parsed.port || 443,
          method: options.method || 'GET',
          path: parsed.pathname + parsed.search,
          headers,
          // 关键：createConnection 必须返回「已完成 TLS 握手」的 socket，
          // 直接返回 CONNECT 的原始 socket 会让 https 以明文发到 443 端口（DeepSeek 报 400 plain http）。
          createConnection: () => tlsMod.connect({
            socket,
            servername: parsed.hostname,
            rejectUnauthorized: false,
          }),
        });
        tlsSocket.on('response', onResponse);
        tlsSocket.on('error', (e) => reject(new Error(`网络请求失败: ${e.message}（代理 ${proxy}）`)));
        if (body) tlsSocket.write(body);
        tlsSocket.end();
      });
      connectReq.on('error', (e) => reject(new Error(`代理 CONNECT 失败: ${e.message}（代理 ${proxy}）`)));
      connectReq.end();
      return;
    } else if (proxy && proxy.startsWith('http://')) {
      // http 目标 + http 代理：绝对 URL 形式直接转发
      const p = new URL(proxy);
      req = httpMod.request({
        hostname: p.hostname,
        port: p.port || 80,
        method: options.method || 'GET',
        path: url,
        headers,
      }, onResponse);
    } else if (proxy) {
      // https 代理（少见）：退化为直连并提示
      reject(new Error(`当前代理 ${proxy} 为 https，暂不支持 https 代理，请改用 http 代理或直连`));
      return;
    } else if (isHttps) {
      req = httpsMod.request({
        hostname: parsed.hostname,
        port: parsed.port || 443,
        method: options.method || 'GET',
        path: parsed.pathname + parsed.search,
        headers,
      }, onResponse);
    } else {
      req = httpMod.request({
        hostname: parsed.hostname,
        port: parsed.port || 80,
        method: options.method || 'GET',
        path: parsed.pathname + parsed.search,
        headers,
      }, onResponse);
    }
    function onResponse(res) {
      let data = '';
      res.setEncoding('utf8');
      res.on('data', (c) => { data += c; });
      res.on('end', () => resolve({ status: res.statusCode, text: data }));
    }
    req.on('error', (e) => reject(new Error(`网络请求失败: ${e.message}${proxy ? `（当前代理 ${proxy}）` : ''}`)));
    if (body) req.write(body);
    req.end();
  });
}

async function callChat(messages, { maxTokens = 1200 } = {}) {
  const cfg = resolveLlmConfig();
  if (!cfg.baseUrl || !cfg.model) throw new Error('到设置里填大模型的地址和模型名');
  if (!cfg.apiKey) throw new Error('到设置里填大模型');
  const res = await proxiedFetch(`${cfg.baseUrl}/chat/completions`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${cfg.apiKey}`,
    },
    body: JSON.stringify({ model: cfg.model, messages, max_tokens: maxTokens }),
  });
  if (res.status >= 400) {
    throw new Error(`大模型接口 ${res.status}: ${res.text.slice(0, 300)}`);
  }
  let data;
  try { data = JSON.parse(res.text); } catch { throw new Error(`大模型返回非 JSON: ${res.text.slice(0, 200)}`); }
  return data.choices?.[0]?.message?.content ?? '';
}

export async function testLlm() {
  const cfg = resolveLlmConfig();
  if (!cfg.baseUrl || !cfg.model) return { ok: false, error: '到设置里填大模型的地址和模型名' };
  if (!cfg.apiKey) return { ok: false, error: '到设置里填大模型' };
  const text = await callChat(
    [{ role: 'user', content: '只回复：ok' }],
    { maxTokens: 8 },
  );
  return { ok: true, model: cfg.model, reply: String(text || '').slice(0, 80) };
}

// 把会议待办整理成上下文供 DeepSeek 参考
function buildActionsContext(db, hitTaskUuids) {
  const lines = [];
  for (const uuid of hitTaskUuids) {
    const m = getMeeting(db, uuid);
    if (!m) continue;
    const actions = listActions(db, uuid);
    if (actions.length === 0) continue;
    lines.push(`## 会议「${m.title}」的待办：`);
    actions.forEach((a, i) => lines.push(`  ${i + 1}. ${a.title}（状态:${a.status}${a.owner ? `,负责人:${a.owner}` : ''}）`));
  }
  return lines.join('\n');
}

// 让模型从"哪些会议提到XX"类问题中拆解检索关键词（含同义/上下位变体），
// 返回关键词数组。代码不猜词——模型的语义理解负责拆词。
async function extractKeywordsByLLM(query) {
  const system = [
    '你是检索规划助手。用户的问题形如"哪些会议提到了/涉及/讨论了某主题"。',
    '请拆解出 2~6 个用于全文检索的关键词（含同义变体、上下位词、用户可能的口语表述）。',
    '要求：只输出 JSON 数组，如 ["AI原生", "AI原生转型", "AI原生企业"]。不要任何其他文字。',
  ].join('\n');
  const user = `问题：${query}\n请输出检索关键词 JSON 数组。`;
  try {
    const raw = await callChat([{ role: 'system', content: system }, { role: 'user', content: user }], { maxTokens: 200 });
    const m = raw.match(/\[[\s\S]*?\]/);
    if (!m) return null;
    const arr = JSON.parse(m[0]);
    if (!Array.isArray(arr)) return null;
    return arr.map((s) => String(s).trim()).filter((s) => s.length > 0 && s.length <= 20).slice(0, 6);
  } catch {
    return null;
  }
}

// 识别问题中的时间范围词，返回对应的 range 值；无时间词返回 null
function detectTimeRange(query) {
  if (/今天|今日/.test(query)) return 'today';
  if (/昨天|昨日/.test(query)) return 'yesterday';
  if (/本周|这周|这一周/.test(query)) return 'thisWeek';
  const m = query.match(/近\s*(\d+)\s*天/);
  if (m) return `lastNdays:${m[1]}`;
  return null;
}

// 归一化答案中的出处引用：把 DeepSeek 写的各种占位符（会议ID缩写/带后缀）替换成完整会议标题，
// 这样前端 linkMap 能按标题精确匹配成可点击链接。
function normalizeCitations(answer, enriched) {
  if (!answer || !enriched || enriched.length === 0) return answer;
  let out = answer;
  // 1. 会议uuid前缀（如 [会议76327569…]、[763275696434…，summary]）→ 完整标题
  for (const h of enriched) {
    const prefix = h.taskUuid.slice(0, 8);
    // [会议76327569…] 或 [76327569…] 或带后缀 [会议76327569…，summary]
    const re = new RegExp(`\\[\\s*(?:会议\\s*)?${prefix}[^\\]]*\\]`, 'g');
    out = out.replace(re, `[${h.title}]`);
  }
  // 2. 兜底：任何 [会议xxx…] 形式的残留（长度>6 的），如果 hits 里有 uuid 前缀匹配则替换
  return out;
}

export async function ask({ query, dbPath, topK = 5, threshold = 0, model, dtype, maxTokens } = {}) {
  // 「提到/提及/讨论过/包含/出现」类问题：让模型拆解检索关键词（含同义变体），
  // 代码按模型给的关键词做字面检索（多词任一命中），再把片段交回模型总结。
  // 分工：模型负责语义理解与拆词，代码只做确定性检索——不靠正则硬拆自然语言。
  const mentionRe = /哪些.*(提到|提及|讨论过|说过|谈到|包含|出现过|有提到|涉及)|(提到|提及|讨论过|说过|谈到|涉及).*(哪些|什么|哪(些|个))/;
  if (mentionRe.test(query)) {
    const kws = await extractKeywordsByLLM(query);
    if (kws && kws.length > 0) {
      const { keywordSearch } = await import('./overview.js');
      const r = keywordSearch(kws.join('，'));
      if (r.count > 0) {
        const lines = [];
        for (const m of r.meetings) {
          const where = m.matchedIn === 'title' ? '标题' : m.matchedIn === 'summary' ? '摘要' : '逐字稿';
          const kwsHit = (m.keywords || []).join('、');
          const d = m.time ? new Date(m.time).toLocaleDateString('zh-CN') : '';
          lines.push(`## 会议「${m.title}」（${d}，命中${where}：${kwsHit}）`);
          if (m.summary) lines.push(m.summary.replace(/!\[.*?\]\(.*?\)/g, '').replace(/<img.*?>/g, '').slice(0, 300));
        }
        const system = [
          '你是公司"会议智能中枢"的助手。用户问的是"哪些会议提到/涉及某主题"。',
          `检索关键词（模型拆解）: ${kws.join('、')}`,
          '下面是明确命中这些关键词的会议记录。请回答：哪些会议确实涉及该主题、各会议中相关内容是什么。',
          '注意：某会议命中关键词不等于真涉及主题——若会议只是顺带提及/无关，请指出或排除，不要硬凑。',
          '引用标注：每条结论后紧跟 [会议标题] 标注出处。',
        ].join('\n');
        const user = [`问题：${query}`, '', lines.join('\n\n'), '请回答。'].join('\n');
        const answer = await callChat([{ role: 'system', content: system }, { role: 'user', content: user }], { maxTokens });
        const hits = r.meetings.map((m) => ({ taskUuid: m.taskUuid, title: m.title, kind: 'keyword', similarity: 1 }));
        return { answer, hits, keyword: kws.join('、') };
      }
    }
  }

  // 只要问题含时间词（今天/昨天/本周/近N天等），就走结构化查询：
  // 把该时间范围内的会议（标题/时间/摘要/待办）作为上下文，避免语义检索答出历史会议。
  const timeRange = detectTimeRange(query);
  if (timeRange) {
    const data = todosByRange(timeRange);
    if (data.meetings.length > 0) {
      const lines = [];
      for (const m of data.meetings) {
        lines.push(`## 会议「${m.title}」（${new Date(m.time).toLocaleString('zh-CN')}）`);
        if (m.summary) {
          const brief = m.summary.replace(/!\[.*?\]\(.*?\)/g, '').replace(/<img.*?>/g, '').slice(0, 400);
          lines.push(brief);
        }
        if (m.actions.length > 0) {
          lines.push('待办：');
          m.actions.forEach((a, i) => lines.push(`  ${i + 1}. ${a.title}（状态:${a.status}）`));
        }
      }
      const system = [
        '你是公司"会议智能中枢"的助手。下面是某时间范围内的真实会议记录（含摘要与待办），请仅基于这些内容回答。',
        '回答要点：优先整理【决策】【共识】【待办】；回答须与该时间范围内的会议相关，不得引用范围外内容。',
        '引用标注：每条结论/待办后紧跟 [会议标题] 标注其出处，如：完成XX（[08-19 光伏项目分阶段建设表调整]）。',
      ].join('\n');
      const user = [
        `问题：${query}`,
        '',
        `【${data.label}（${new Date(data.start).toLocaleDateString('zh-CN')} ~ ${new Date(data.end - 1).toLocaleDateString('zh-CN')}）的会议记录】`,
        lines.join('\n\n'),
        '请回答上述问题。',
      ].join('\n');
      const answer = await callChat(
        [{ role: 'system', content: system }, { role: 'user', content: user }],
        { maxTokens },
      );
      // 结构化路径：把该时间范围内的会议作为 hits 返回（供前端把 [会议标题] 渲染成可点击链接）
      const rangeHits = data.meetings.map((m) => ({
        taskUuid: m.taskUuid,
        title: m.title,
        kind: 'meeting',
        similarity: 1,
      }));
      return { answer, hits: rangeHits, structured: true };
    }
  }

  // 1. 本地语义检索
  const hits = await semanticSearch({ dbPath, query, topK, threshold, model, dtype });

  // 给每个命中会议分配编号（去重），供 DeepSeek 用 [N] 精确引用
  const db0 = open(dbPath);
  const seenUuids = new Set();
  const numbered = [];
  for (const h of hits) {
    if (seenUuids.has(h.task_uuid)) continue;
    seenUuids.add(h.task_uuid);
    const m = getMeeting(db0, h.task_uuid);
    numbered.push({
      taskUuid: h.task_uuid,
      title: m ? m.title : '未知会议',
      kind: h.kind,
      similarity: h.similarity,
      chunk: h.chunk_text,
      idx: numbered.length + 1,
    });
  }
  db0.close();

  const contextParts = [];
  for (const n of numbered) {
    contextParts.push(`【${n.idx}| ${n.kind} · ${n.title}】\n${n.chunk}`);
  }

  const hitUuids = new Set(numbered.map((n) => n.taskUuid));
  const db = open(dbPath);
  const actionsCtx = buildActionsContext(db, hitUuids);
  db.close();
  if (actionsCtx) contextParts.push(actionsCtx);

  if (contextParts.length === 0) {
    return { answer: '本地没有检索到相关会议内容，请换一个问法。', hits: [] };
  }

  const system = [
    '你是公司"会议智能中枢"的助手。你会收到带编号的检索片段，请基于这些内容回答。',
    '回答要点：优先给出【决策】【共识】【待办】等结构化结论；若检索内容不足以回答，明确说明缺口。',
    '引用标注：每条结论/要点后用 [编号] 标注其出处，编号必须来自片段开头【N| …】中的数字，如：完成XX（[1]）。',
  ].join('\n');

  const user = [
    `问题：${query}`,
    '',
    '以下是命中的会议内容：',
    '--------',
    contextParts.join('\n\n'),
    '--------',
    '请回答上述问题。',
  ].join('\n');

  const answer = await callChat(
    [{ role: 'system', content: system }, { role: 'user', content: user }],
    { maxTokens },
  );

  // 把答案中的 [N] 编号引用替换为完整会议标题（精确映射，不受 uuid 前缀/标题拼写影响）
  let finalAnswer = answer;
  for (const n of numbered) {
    finalAnswer = finalAnswer.replace(new RegExp(`\\[${n.idx}\\]`, 'g'), `[${n.title}]`);
  }
  // 兜底：残留的 [会议uuid前缀…] 用完整 uuid 精确匹配替换
  for (const n of numbered) {
    const re = new RegExp(`\\[\\s*(?:会议\\s*)?${n.taskUuid}[^\\]]*\\]`, 'g');
    finalAnswer = finalAnswer.replace(re, `[${n.title}]`);
  }

  const enriched = numbered.map((n) => ({
    taskUuid: n.taskUuid,
    title: n.title,
    kind: n.kind,
    similarity: n.similarity,
  }));

  return { answer: finalAnswer, hits: enriched };
}

// 基于逐字稿做「可直接用于决策追踪」的会议记录。
// 系统提示词：server/会议记录提炼提示词.md（本机 ~/.dsh/meetings/summarize-prompt.md 可覆盖）。
// 称呼/术语：server/glossary.default.json + ~/.dsh/meetings/glossary.json。
export async function summarizeTranscript({ taskUuid, maxTokens = 8192 } = {}) {
  const db = open();
  const m = getMeeting(db, taskUuid);
  if (!m) { db.close(); return { error: '未找到会议' }; }
  const paras = db.prepare(`SELECT chunk_text FROM chunks WHERE task_uuid=? AND kind='transcript' ORDER BY id`).all(taskUuid);
  const acts = db.prepare(`SELECT title,status FROM actions WHERE task_uuid=?`).all(taskUuid);
  const keywords = m.keywords_json ? (() => { try { return JSON.parse(m.keywords_json) || []; } catch { return []; } })() : [];
  db.close();

  // 清洗逐字稿：过滤纯语气词/无意义段落，拼接为文本
  const noiseRe = /^(【[^】]*】)?\s*(嗯|啊|哦|噢|呃|喂|哈|哎|唉|hmm|hm|um|uh|对|是|好|行|ok|okay|yes|no|嗯嗯|啊啊|好好|对对|是是|谢谢|不|没有)[。，！？\s,.!?]*$/i;
  const meaningful = paras
    .map((p) => p.chunk_text)
    .filter((t) => !noiseRe.test(t))
    .filter((t) => t.replace(/【[^】]*】/g, '').trim().length >= 4);
  const fullText = meaningful.join('\n');

  if (fullText.length < 50) {
    return { error: '该会议逐字稿内容过少，无法总结', meeting: m.title, paras: paras.length, meaningful: meaningful.length };
  }

  const glossary = loadGlossary();
  // 本场关键词并入术语表，供同音纠正（不写回文件）
  if (Array.isArray(keywords)) {
    for (const kw of keywords) {
    const s = typeof kw === 'string'
      ? kw.trim()
      : String((kw && (kw.name || kw.keyword || kw.value)) || '').trim();
      if (s && !glossary.terms.some((t) => t.name.toLowerCase() === s.toLowerCase())) {
        glossary.terms.push({ name: s, aliases: [] });
      }
    }
  }
  const speakers = extractSpeakers(meaningful);
  const resolved = resolveGlossary({
    glossary,
    attendees: m.attendees,
    speakers,
    title: m.title,
    actions: acts,
    transcript: fullText,
  });

  const system = loadSummarizePrompt();
  const transcriptSlice = fullText.slice(0, 24000);
  const user = formatSummarizeInputs({ meeting: m, resolved, actions: acts, transcript: transcriptSlice });

  const answer = await callChat(
    [{ role: 'system', content: system }, { role: 'user', content: user }],
    { maxTokens },
  );
  return { meeting: m.title, taskUuid, paras: paras.length, meaningful: meaningful.length, summary: answer };
}
