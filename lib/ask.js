  ...
  return new Promise((resolve, reject) => {
    ...
    if (proxy && proxy.startsWith('http://') && isHttps) {
      ...
      connectReq.on('connect', (res, socket) => {
        ...
        // 关键：CONNECT 后拿到的 socket 是裸 TCP，必须用 tls.connect 显式完成 TLS 握手，
        // 再交给 https.request。直接 createConnection: () => socket 会让请求以明文发出
        // （DeepSeek 400: plain http request sent to https port）。
        const tlsSocket = tlsConnect({
          socket,
          servername: parsed.hostname,
        });
        const tlsReq = httpsMod.request({
        ...
        tlsReq.on('response', onResponse);
        tlsReq.on('error', (e) => reject(new Error(`网络请求失败: ${e.message}（代理 ${proxy}）`)));
