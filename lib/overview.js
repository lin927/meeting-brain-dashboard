...
function mapMeetingRow(m, actionCount = 0) {
  return {
...
function collectTags(db) {
  const rows = db.prepare(
    `SELECT tags_json FROM meetings WHERE tags_json IS NOT NULL AND tags_json != '' AND tags_json != '[]'`
  ).all();
  const seen = new Set();
  const tags = [];
  for (const r of rows) {
...
export function queryMeetings(opts = {}) {
  ...
  const tag = String(opts.tag || '').trim().slice(0, 40);
  ...
  const rows = db.prepare(
    `SELECT task_uuid, title, start_time, visibility, source, scope, tags_json
  ...
  const tags = cursor ? [] : collectTags(db);
  db.close();
  return {
    items: rows.map((m) => mapMeetingRow(m, counts.get(m.task_uuid) || 0)),
    total,
    nextCursor,
    tags,
  };
