...
import { inferProvider, canRefresh, canWriteback, providerMeta } from './providers/ids.js';
...
function mapMeetingRow(m, actionCount = 0, glossary) {
  ...
  const durationMs = Number(m.duration_ms) || 0;
  const span = (Number(m.end_time) && Number(m.start_time) && Number(m.end_time) > Number(m.start_time))
    ? (Number(m.end_time) - Number(m.start_time))
    : 0;
  return {
    taskUuid: m.task_uuid,
    title: String(m.title || '').replace(/^会议录制：/, ''),
    time: m.start_time,
    durationMs: durationMs > 0 ? durationMs : (span > 0 ? span : null),
    visibility: m.visibility || 'private',
    source: m.source || 'mine',
    provider: m.provider || inferProvider(m),
...
export function queryMeetings(opts = {}) {
  ...
  const filter = opts.filter === 'company' || opts.filter === 'pending' ? opts.filter : 'all';
  ...
  if (filter === 'pending') {
    where.push(`IFNULL(visibility, 'private') != 'company'`);
    where.push(`IFNULL(scope, '') != '个人'`);
  }
  if (provider === 'import') {
    where.push(`(IFNULL(provider,'') = 'import' OR IFNULL(source,'') = 'import' OR task_uuid LIKE 'import-%')`);
  } else if (provider === 'dingtalk' || provider === 'feishu' || provider === 'tencent') {
    where.push(`IFNULL(provider, 'dingtalk') = ?`);
  ...
  const rows = db.prepare(
    `SELECT task_uuid, title, start_time, end_time, duration_ms, visibility, source, provider, scope, tags_json, project_name, project_code
...
export function meetingDetail(taskUuid) {
  ...
  return {
    ...
    providerLabel: providerMeta(inferProvider(m)).label,
    canRefresh: canRefresh(m),
    writebackCapable: canWriteback(m),
    summary: m.summary,
