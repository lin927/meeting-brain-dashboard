...
export function keywordSearch(keyword) {
  ...
  for (const m of byText) push(m, m.title && String(m.title).includes(kw) ? 'title' : 'summary');
  for (const u of chunkUuids) {
    const m = db.prepare(`SELECT task_uuid, title, start_time, summary FROM meetings WHERE task_uuid = ?`).get(u);
    if (m) push(m, 'transcript');
  }
  db.close();
  return { keyword: kw, count: meetings.length, meetings };
}
