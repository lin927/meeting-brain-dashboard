...
export function updateMeetingFields(db, taskUuid, fields) {
  const allowed = ['title', 'attendees', 'summary', 'visibility', 'published_at', 'company_doc_id', 'company_dataset_id', 'card_json', 'edited_at', 'scope', 'tags_json', 'source', 'deep_summary', 'summary_edited_at', 'transcript_edited_at', 'scope_source', 'project_name', 'project_code'];
  const sets = [];
  const vals = [];
  for (const k of allowed) {
    if (fields[k] === undefined) continue;
