...
export function open(override) {
  ...
  ensureColumn(db, 'meetings', 'scope_source', "scope_source TEXT DEFAULT ''");
  ensureColumn(db, 'meetings', 'project_name', "project_name TEXT DEFAULT ''");
  ensureColumn(db, 'meetings', 'project_code', "project_code TEXT DEFAULT ''");
  ensureColumn(db, 'meetings', 'provider', "provider TEXT DEFAULT ''");
  ensureColumn(db, 'meetings', 'remote_id', "remote_id TEXT DEFAULT ''");
  ensureColumn(db, 'meetings', 'remote_json', 'remote_json TEXT');
  backfillProviders(db);
...
function backfillProviders(db) {
  db.exec(`
    UPDATE meetings SET provider = 'import'
    WHERE IFNULL(provider,'') = '' AND (IFNULL(source,'') = 'import' OR task_uuid LIKE 'import-%');
    UPDATE meetings SET provider = 'feishu', remote_id = substr(task_uuid, 4)
    WHERE IFNULL(provider,'') = '' AND task_uuid LIKE 'fs:%';
    UPDATE meetings SET provider = 'tencent', remote_id = CASE
      WHEN task_uuid LIKE 'tm:rf:%' THEN substr(task_uuid, 7)
      WHEN task_uuid LIKE 'tm:m:%' THEN substr(task_uuid, 6)
      ELSE substr(task_uuid, 4)
    END
    WHERE IFNULL(provider,'') = '' AND task_uuid LIKE 'tm:%';
    UPDATE meetings SET provider = 'dingtalk', remote_id = task_uuid
...
export function upsertMeeting(db, m) {
  db.prepare(`
    INSERT INTO meetings (task_uuid, title, start_time, end_time, duration_ms, url, source,
                          ...
                          provider, remote_id, remote_json)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ON CONFLICT(task_uuid) DO UPDATE SET
      ...
      duration_ms=COALESCE(excluded.duration_ms, meetings.duration_ms),
      ...
      provider=COALESCE(NULLIF(meetings.provider,''), excluded.provider),
      remote_id=COALESCE(NULLIF(meetings.remote_id,''), excluded.remote_id),
      remote_json=COALESCE(meetings.remote_json, excluded.remote_json)
  `).run(
    ...
    m.provider ?? '', m.remote_id ?? '', m.remote_json ?? null,
  );
}
...
/** 只删本机副本。来源侧听记不动；uuid 记入 deleted_meetings，避免下次同步再入库。 */
...
/** 本机「总结」工作副本。不覆盖来源侧纪要 summary；pull 也不会改这一列。 */
...
export function updateMeetingFields(db, taskUuid, fields) {
  const allowed = ['title', 'attendees', 'summary', 'visibility', 'published_at', 'company_doc_id', 'company_dataset_id', 'card_json', 'edited_at', 'scope', 'tags_json', 'source', 'deep_summary', 'summary_edited_at', 'transcript_edited_at', 'scope_source', 'project_name', 'project_code', 'provider', 'remote_id', 'remote_json', 'duration_ms', 'end_time'];
  const sets = [];
  const vals = [];
  for (const k of allowed) {
    if (fields[k] === undefined) continue;
