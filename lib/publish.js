...
export async function repairPublishedMetadata() {
  ...
  for (const meeting of rows) {
    ...
    try {
      const docs = await listDocuments(kb, meeting.company_dataset_id, {
        page: 1,
        page_size: 100,
        orderby: 'update_time',
        desc: true,
      });
      const doc = docs.find((d) => d.id === meeting.company_doc_id);
      if (!doc) {
        failed += 1;
        continue;
      }
      ...
      const meta = buildCompanyMetadata(meeting, {
        datasetId: meeting.company_dataset_id,
      ...
      if (metaCovered(fields, existing)) continue;
      await updateDocumentMetaFields(kb, meeting.company_dataset_id, meeting.company_doc_id, fields);
      updated += 1;
    } catch (err) {
      failed += 1;
      pushLog('publish', `补写「${meeting.title}」元数据失败：${err.message}`, 'error');
    }
  }
  if (updated) pushLog('publish', `已补写 ${updated} 场会议的知识库元数据`);
