...
- **更新本仓库代码后**：顶栏出现「有更新」时点一下会自动拉取并重启；也可 Mac 执行 `bash scripts/restart.sh`，Windows 执行 `powershell -ExecutionPolicy Bypass -File scripts\restart.ps1`。本机有未提交改动时不会自动覆盖。
